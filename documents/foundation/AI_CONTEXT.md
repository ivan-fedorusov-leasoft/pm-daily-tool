...
-   Team Leads review and discuss only what requires attention.
-   Game Page is the source of truth for a single project.
-   Radar is a lightweight overview only.
-   Timeline is postponed.
-   Search is postponed.
-   Daily Host is not the same as Team Lead.
-   Daily Host controls the meeting, not project permissions.
...
- Being a Daily Host does not grant Manager permissions.
...
-   If documentation and implementation conflict, update the
    implementation unless documentation is intentionally changed.