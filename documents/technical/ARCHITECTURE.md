...
- Daily Tool reuses the host app's infrastructure (auth, hosting, database) wherever it safely can, and isolates its own concerns behind a `radar_` naming prefix and a dedicated route segment.
...
| Identity | `getCurrentUserKey()` → email or `"local"` | New `radar_users` table keyed by email; upserted on sign-in |
| Roles | None (no role concept in gc-games-dashboard) | New `radar_users.role` column: developer / manager / admin |
| Layout | No shared layout currently | New `src/app/radar/layout.tsx` — shared nav for BE Radar section |
| DB access | Service role key, `getSupabase()` singleton | Same pattern — `supabase.from('radar_*').select(...)` |
| Hosting | Vercel + Supabase `hstvuhqqbhzsgkzvgntm` | Shared as-is, no new infra |
| Route gating | NextAuth middleware — unauthenticated → `/login` | Applies to `/radar/*` automatically if auth configured |
...
- Types: defined locally in `src/lib/radar/types.ts` — no generated `database.types.ts` needed
...
src/
  app/
    daily/
      ...
      page.tsx              # redirects to /radar/today
      today/
        page.tsx
        TodayWorkClient.tsx
        actions.ts
  ...
  lib/radar/
    ...
    users.ts                # getCurrentDailyUser() — upserts radar_users on sign-in
    games.ts                # radar_games + radar_game_stage_dates queries
    notes.ts                # radar_notes queries
    change-requests.ts      # radar_change_requests queries

supabase/migrations/
  0003_radar_tool.sql       # radar_users + all radar_* tables (next after 0002)
```
...
## `lib/radar/*` (business logic + data access)
...
## Roles (stored in `radar_users`)

Every Daily Tool role is stored in `radar_users.role`. There are no separate Daily Tool accounts — identity comes from the NextAuth Google session.
...
Role is set by admin directly in the DB. On first sign-in, user is upserted into `radar_users` with `role = 'developer'`.
...
Note: "manage users" here means Daily Tool–scoped settings (e.g. `radar_role` overrides), not `gc-pm-automation` user management, which remains under `gc-pm-automation`'s own `super_admin`/Settings page.
...
Daily Host does not receive Manager/Admin permissions, regardless of their derived `radar_role`.
...
Derived `radar_role` determines permissions.
...
Stored in PostgreSQL, in the Supabase project backing `gc-games-dashboard` (`hstvuhqqbhzsgkzvgntm`), in `radar_`-prefixed tables.

Tables:

- radar_users (owned by Daily Tool — new)
- radar_games
- radar_game_stage_dates
- radar_notes
- radar_change_requests
- radar_stage_history
- radar_pull_requests (v2)
...
- Introduce the originally-planned `server/{services,repositories,permissions}` layered structure — use flat `lib/radar/*` + colocated `actions.ts` instead.
